...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = 'c40bd181-02c5-43a5-8387-b84efff99e1f'; // ID de Beauty Salón
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Beauty Salón';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '55733244';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'yoicelysg@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Beauty Salón!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'beautysalon';
};
...
console.log('✅ config-negocio.js listo para Beauty Salón');
